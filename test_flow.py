    ...
    while True:
        ...
        for item_bbox in list_bboxs:
            ...
            if track_id not in trackers:
                trackers[track_id] = [center]
            else:
                trackers[track_id].append(center)

        # 绘制轨迹
        for track_id, positions in trackers.items():
            for i in range(1, len(positions)):
                # 使用不同颜色绘制每个轨迹
                color = (255 * (track_id % 3 == 0), 255 * (track_id % 3 == 1), 255 * (track_id % 3 == 2))
                cv2.line(output_image_frame, positions[i - 1], positions[i], (0,0,255), 2)
        ######################################
        # 输出图片
